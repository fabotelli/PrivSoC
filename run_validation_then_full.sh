#!/usr/bin/env bash
# Two-stage driver (verbatim from cube_stack, paths renamed):
...
#   GATE C: if the validation policy shows ANY real learning signal
...
#                   the full pipeline's skip-guard on dataset_mycobot_stack.npz
#                   is untouched and the real 8000-ep collection runs fresh.
set -uo pipefail
...
bash gpu_claim.sh python3 train_bc_mycobot_stack.py \
    --data dataset_val.npz --out bc_mycobot_val.pt \
...
[ -f bc_mycobot_val.pt ] || { echo "ABORT: no bc_mycobot_val.pt after training"; exit 1; }
