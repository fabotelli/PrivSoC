"""
collect_dataset_mycobot_stack.py
=============================
...
driving ``MycobotStackEnv`` + ``MycobotStackSolver`` (a single pick-and-place per
episode: pick the farther cube, stack it on the nearer cube).

Differences vs the dice-sort collector:
  * Imports MycobotStackEnv / MycobotStackSolver.
...
from mycobot_stack_env import MycobotStackEnv, ARM_JOINTS
from mycobot_stack_solver import MycobotStackSolver


# Metadata only (describes policy_cam for the npz header; rendering uses the
# named policy_cam camera in the scene XML).
CAM_LOOKAT = np.array([0.1725, 0.0, 0.16])
CAM_DISTANCE = 0.361
CAM_AZIMUTH = 127.3
CAM_ELEVATION = -32.4
...
def main():
    ...
    ap.add_argument("--out", default="dataset_mycobot_stack.npz")
