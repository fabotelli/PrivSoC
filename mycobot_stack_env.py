"""
mycobot_stack_env.py
====================

Wrapper around ``mycobot_scene_cube_stack.xml`` for the two-cube STACKING task
on the Elephant Robotics myCobot 280.  Port of ``cube_stack_env.py`` (LeArm)
with ONLY the arm-specific constants changed:

  * ``ARM_JOINTS``: the six Elephant joint names (``joint2_to_joint1`` ...
    ``joint7_to_joint6`` = J1..J6, 1:1 with ``pymycobot`` joint indices).
  * ``ARM_ACTS``: ``a_j1..a_j6``; gripper is a slide joint (parallel jaws),
    so ``GRIPPER_OPEN``/``GRIPPER_CLOSE`` are metres of jaw travel.
  * wrist-roll joint index 4 -> 5 (J6 spins the flange about the approach
    axis; the pinch site sits on that axis so the roll is a free DOF pinned
    after IK convergence, exactly like the LeArm's wrist_roll).
  * ``home_seed`` / ``home_xy`` / cube zone re-centred for the 280's reach
    (see DECISIONS.md #3), ``table_top_z`` unchanged at 0.15.
  * ``BASE_HEIGHT_OFFSET`` (calibration knob, default 0.0) is added to the
    arm base z at load time.

Everything else (two identical red cubes, spawn rejection sampling, privileged
accessors, stack-success check, DLS IK) is verbatim from the LeArm env.
The student policy only ever sees the side camera + joint angles.
"""
...
ARM_ACTS = ["a_j1", "a_j2", "a_j3", "a_j4", "a_j5", "a_j6"]
GRIP_ACT = "a_grip"

# Parallel gripper: slide-joint travel in metres (inner pad faces sit at
# +-(s + 0.0015)).  OPEN = 33 mm opening; CLOSE over-travels ~12 mm past cube
# contact for clamp force (position actuator supplies the grip force).
GRIPPER_OPEN = 0.016
GRIPPER_CLOSE = -0.001
APPROACH_DOWN = np.array([0.0, 0.0, -1.0])

# Wrist-roll = 6th joint (J6, joint7_to_joint6) for the 280 (index 4 -> 5).
WRIST_ROLL_IDX = 5

# Calibration knob: vertical offset of the real base plate vs the modelled one
# (the JN variant's housing differs from other 280 bases).  Applied to the
# base body z at load.  See HANDOVER.md.
...
# Minimum centre-to-centre spawn separation (LeArm-verified pixel floor value;
# re-verified for this scene by check_pixel_floor in Step 4).
CUBE_MIN_SEP = 0.065

# Cube spawn zone.  The wrist-down feasible annulus at base_z=0.15 is
# r in [0.15, 0.21] (sweep_base_height.py / sweep_zone_rect.py: 0/147 IK
# failures on a 7x7 grid x 3 heights for this rectangle, the largest of the
# candidates at 63 cm^2).  See DECISIONS.md #3.
CUBE_X_LO, CUBE_X_HI = 0.150, 0.195
CUBE_Y_LO, CUBE_Y_HI = -0.070, 0.070


class MycobotStackEnv:
    """myCobot 280 two-cube stacking environment with privileged state access."""

    def __init__(self, xml_path: str | None = None):
        if xml_path is None:
            xml_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                    "mycobot_scene_cube_stack.xml")
        self.model = mujoco.MjModel.from_xml_path(xml_path)
        if BASE_HEIGHT_OFFSET != 0.0:
            bid = mujoco.mj_name2id(self.model, mujoco.mjtObj.mjOBJ_BODY,
                                    "base_link")
            self.model.body_pos[bid][2] += BASE_HEIGHT_OFFSET
        ...
        # Elbow-bent, wrist-down seed (probe_kinematics.py).
        self.home_seed = np.array([0.0, 0.7, -1.4, -0.85, 0.0, 0.0])
        ...
        self.home_xy = np.array([0.1725, 0.0])
    ...
    def reset(self, seed: int | None = None) -> None:
        ...
        # Home pose (pinch 60 mm above the zone centre, jaws open).
        home_q = self.solve_ik(np.array([0.1725, 0.0, 0.210]),
        ...
        for c in CUBES:
            x, y = self._sample_xy_separated(
                CUBE_X_LO, CUBE_X_HI, CUBE_Y_LO, CUBE_Y_HI, min_sep=CUBE_MIN_SEP,
    ...
    # --------------------------------------------------------------------- #
    #  IK (verbatim from cube_stack_env; only the pinned-roll index changes)
    # --------------------------------------------------------------------- #
    def solve_ik(self,
        ...
        if pin_wrist_roll is not None:
            d.qpos[self.arm_qpos[WRIST_ROLL_IDX]] = np.clip(
                pin_wrist_roll, self.arm_range[WRIST_ROLL_IDX, 0],
                self.arm_range[WRIST_ROLL_IDX, 1])
